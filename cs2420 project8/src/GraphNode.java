...
public class GraphNode {
    ...
    public void addEdge(int v1, int v2, int capacity,int cost){
        succ.addFirst( new EdgeInfo(v1,v2,capacity,cost,0) );
    }
    ...
    public int getFlow(int destination) {
        Iterator<EdgeInfo> itr = succ.iterator();
        while (itr.hasNext()){
            EdgeInfo e = itr.next();
            if (e.to == destination)
    ...
    public int getResidualFlow(int destination) {
        return this.getCapacity(destination) - this.getFlow(destination);
    }

    public void addFlow(int destination, int amount) {
        Iterator<EdgeInfo> itr = succ.iterator();
        while (itr.hasNext()) {
            EdgeInfo e = itr.next();
            if (e.to == destination) {
